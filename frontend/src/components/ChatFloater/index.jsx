...
export default function ChatFloater({ onClose, onExpand }) {
  ...
  // When a friend is clicked, find/create chat and select it
  ...
  // Load messages once a chat is selected
  useEffect(() => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      // real message arrives via socket; remove temp
    ...
    >
      ...
      {selectedChat ? (
        <>
          ...
          />
          <span className="font-bold truncate">{selectedChat.displayName}</span>
        </>
      ) : (
        <span className="text-lg font-bold tracking-wide">Chats</span>
      )}
      <div className="flex-1" />
      <button onClick={onExpand} className="hover:text-gray-200 mr-2" title="Expand">
      ...
      <button onClick={onClose} className="hover:text-gray-200" title="Close">
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const InputBar = () =>
    selectedChat ? (
      <form
        onSubmit={handleSend}
        className="flex items-center px-3 py-2 border-t bg-white flex-shrink-0"
  ...
  if (!selectedChat) {
    return (
      <div className="flex flex-col h-full min-h-0 overflow-hidden rounded-xl bg-white">
        <Header />
  ...
  // ----------- CHAT MODE (only messages scroll) -----------
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden rounded-xl bg-white">
      <Header />
