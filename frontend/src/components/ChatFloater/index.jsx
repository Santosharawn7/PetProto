...
export default function ChatFloater({ onClose, onExpand }) {
  ...
  // When you pick a friend, get (or create) chat with them
  const handleFriendClick = async (friend) => {
    ...
    try {
      ...
      let chat = (chatsRes.data.chats || []).find(
        (c) =>
          !c.isGroup && c.participants && c.participants.includes(friend.uid)
      );
      if (!chat) {
      ...
      >
        ...
        >
          <path
            ...
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M4 8V6a2 2 0 0 1 2-2h2m8 0h2a2 2 0 0 1 2 2v2m0 8v2a2 2 0 0 1-2 2h-2m-8 0H6a2 2 0 0 1-2-2v-2"
          />
        </svg>
      </button>
      <button
  ...
  if (!selectedChat) {
    return (
      <div className="flex flex-col h-full rounded-xl shadow-lg bg-white">
        {header}
            ...
            <ul>
              {friends.map((friend) => (
                <li
                  ...
                  hover:bg-gray-200
                  active:bg-purple-100 transition-all duration-200 border-b border-gray-200  last:border-b-0"
  ...
  // Chat view: header & input sticky, only messages scroll!
  return (
    <div className="flex flex-col h-full relative rounded-xl bg-white shadow-lg overflow-hidden">
      {header}
      <div
      ...
      >
        {loadingMsgs ? (
          ...
          (() => {
            // ---- DATE GROUPING & TIMESTAMP RENDER ----
            let lastDate = null;
                    ...
                    <div className={`max-w-xs ${isMe ? "ml-auto" : ""}`}>
                      ...
                      >
                        ...
                        {(idx === messages.length - 1 ||
                          messages[idx + 1]?.from !== m.from ||
                          new Date(messages[idx + 1]?.sentAt) - msgDate >
                            60000) && (
                          <div
