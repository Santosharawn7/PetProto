...
export default function ChatFloater({ onClose, onExpand }) {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        const res = await axios.get(`${API_URL}/chats/${selectedChat.chatId}/messages`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  // ----------- HEADER (Sticky) -----------
  let header = (
    <div className=" rounded-2xl flex items-center px-4 py-3 border-b bg-gradient-to-r from-pink-500 to-purple-500 sticky top-0 z-20">
      {selectedChat && (
        <button
      ...
      {selectedChat ? (
        <>
          ...
          />
          <span className="text-white text-base font-bold">{selectedChat.displayName}</span>
        </>
      ) : (
        <span className="text-white text-lg font-bold tracking-wide">Chats</span>
      )}
      <div className="flex-1" />
      <button onClick={onExpand} className="text-white hover:text-gray-200 mr-2" title="Expand">
  ...
  return (
    <div className="flex flex-col h-full relative !rounded-2xl">
      {header}
