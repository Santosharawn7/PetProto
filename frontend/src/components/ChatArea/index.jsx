...
export default function ChatArea({ chatId, currentChat, onBackToChats, socket, isMobile }) {
  ...
  useEffect(() => { if (chatId) fetchMessages(); }, [chatId]);
  ...
  const handleSend = async (e) => {
    ...
    const temp = { id: `temp_${Date.now()}`, from: currentUid, text, sentAt: new Date().toISOString(), authorName: 'You', isTemporary: true };
    ...
    try {
      const token = await auth.currentUser.getIdToken();
      await axios.post(`${API_URL}/chats/${chatId}/messages`, { text }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    <div className="flex flex-col h-full min-h-0 bg-white">
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 py-4 bg-white border-b border-gray-200 flex-shrink-0">
        <div className="flex items-center space-x-3 min-w-0 flex-1">
          <button onClick={onBackToChats} className="md:hidden p-2 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0">
      ...
      {/* Messages */}
      <div className="flex-1 min-h-0 overflow-y-auto px-4 sm:px-6 py-4 bg-gray-50">
        {loadingMessages ? (
      ...
      {/* Input */}
      <div className="px-4 sm:px-6 py-4 bg-white border-t border-gray-200 flex-shrink-0">
        <form onSubmit={handleSend} className="flex items-center space-x-3">
          ...
          <button
            type="submit"
            disabled={!input.trim()}
            className={`p-3 rounded-full transition-all flex-shrink-0 ${input.trim() ? 'bg-gradient-to-r from-pink-500 to-purple-600 text-white shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
          >
